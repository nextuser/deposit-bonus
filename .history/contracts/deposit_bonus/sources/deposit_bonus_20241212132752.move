...
public fun get_bonus_period(bh :&BonusHistory,hour : u16) : BonusPeriod{
    
    let node = bh.history.borrow(hour);
    *node 
}

entry fun entry_get_bonus_period(bh :&BonusHistory,hour : u16){
    emit(get_bonus_period(bh, hour));
}
...
const DATE_MS : u64 = 24 * 3600 * 1000;
public(package) fun allocate_bonus(storage : &mut Storage,
    ...
    let date = (time_ms / DATE_MS) as u16;
    bonus_history.history.push_front(date,period);
    bonus_history.days.push_back(date as u16);
