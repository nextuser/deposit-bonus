...
use sui_system::sui_system::{Self,SuiSystemState,request_withdraw_stake_non_entr,request_add_stake_non_entry};
    ...
    while(!vector::is_empty(&storage.staked_suis)){
        ...
        if( curr_amount > need  ){
            ...
            return split_exact_balance(storage,merge_balance , amount)
              ...
              let balance = request_withdraw_stake_non_entry(wrapper,staked,ctx); 
              merge_balance.join(balance);
...
fun withdraw_all_from_stake(storage :&mut Storage, 
    ...
    while(!vector::is_empty<StakedSui>(&storage.staked_suis)){
        ...
        let b = request_withdraw_stake_non_entry(wrapper,staked,ctx); 
        balance.join(b);
    };
