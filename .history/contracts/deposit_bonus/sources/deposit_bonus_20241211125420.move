...
#[test_only]
fun test_init() : (Clock,Random,Scenario,AdminCap,OperatorCap)
{
    ...
    let mut effect = tests::next_tx(&mut sc,@0xa);
    ...
    let admin_cap = tests::take_from_address<AdminCap>(&sc, @0xa);
    let operator_cap = tests::take_from_address<OperatorCap>(&sc, @0xa);
    assign_operator(&admin_cap,operator_cap, @0xb);

    (clock,random,sc,admin_cap,operator_cap)
...
#[test]
fun test_hit_range(){
    let  (clock,random,sc,admin_cap,operator_cap) = test_init();
    tests::next_tx(&mut sc, @0xc);
    
    
    test_finish(clock, random, admin_cap, operator_cap, sc);
...
#[test]
fun test_point(){
    let  (clock,random,mut sc,admin_cap,operator_cap) = test_init();
    tests::next_tx(&mut sc, @0xc);
    let mut storage = tests::take_shared<Storage>(&sc);
    let mut i = 0;
    while(i < 10){
