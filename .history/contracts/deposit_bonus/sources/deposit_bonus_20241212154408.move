...
public struct BonusHistory has key{
    id : UID,
    history : LinkedTable<u32,BonusPeriod>,//key  time_ms/(1 day ms)
    hours : vector<u32>
...
entry fun get_bonus_records(bh :&BonusHistory,hour : u32) : vector<BonusRecord>{
    let node : &BonusPeriod = bh.history.borrow(hour);
    node.get_bonus_list()
}

entry fun get_bonus_hours(bh :&BonusHistory) : vector<u32>{
    bh.hours
}
/**
...
public(package) fun allocate_bonus(storage : &mut Storage,
    ...
    bonus_history.history.push_front(time,period);
    bonus_history.hours.push_back(time );
