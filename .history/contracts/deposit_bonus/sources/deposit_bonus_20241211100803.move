...
fun allocate_bonus(storage : &mut Storage,
    ...
    while(!option::is_none(node)){
        ...
        // let user_share = linked_table::borrow<address,UserShare>(&storage.user_shares,addr);
        let pay = (user_share.user_share * balance_value )/ storage.total_shares;
        let record = bonus::create_user_record(addr ,  gain as u64,
