...
public  fun withdraw(clock: &Clock,storage: & mut Storage,wrapper: &mut SuiSystemState,
    ...
    if(user_share.bonus > amount){
        ...
        let bonus = withdraw_bonus(storage, ctx);

        let split = bonus.split(amount);

    }
    else if(user_share.bonus == amount){
        return  withdraw_bonus(storage, ctx);
    }


    let user_money = share_to_money(storage, user_share.share_amount);
    assert!(user_money + bonus.value() >= amount ,err_consts::share_not_enough!() );
