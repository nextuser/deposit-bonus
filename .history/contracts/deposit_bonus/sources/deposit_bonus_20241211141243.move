...
use sui::test_utils;
use sui_system::sui_system_tests::{set_up_sui_system_state,Self}

#[test_only]
fun test_init() : (Clock,Random,Scenario)
{
    ...
    tests::return_to_address(@0xa,admin_cap, );
    (clock,random,sc)
...
#[test]
fun test_hit_range(){
    let  (clock,random,mut sc) = test_init();
    tests::next_tx(&mut sc, @0xc);
    
    
    test_finish(clock, random,sc);
