...
public withdraw_bonus() :Balance<SUI> {
        let sender = ctx.sender();
    let user_share = storage.user_shares.borrow_mut(sender);
    assert!( user_share.bonus <= storage.bonus_balance.value());

    let coin = coin::from_balance(storage.bonus_balance.split(user_share.bonus),
                                        ctx);
    user_share.bonus = 0;
}
entry fun withdraw_bonus(storage : &mut Storage, ctx : &mut TxContext){
