...
fun reduce_share_after_withdraw(storage : &mut Storage,money : u64, withdraw_share : u64, sender : address)
{
...
public  fun withdraw(clock: &Clock,storage: & mut Storage,wrapper: &mut SuiSystemState,
    ...
    //calcualte share before withdraw stake ( total_staked will change when withdraw)
    ...
    reduce_share_after_withdraw(storage,balance.value(),withdraw_share,sender);
    balance
}
...
fun test_deposit(){
    ...
    tests::next_tx(&mut sc, USER2_ADDR);
    {
        ...
        let user_share = get_share_by_user(&storage, USER2_ADDR);
        assert_eq(share , amount2 );
    };
